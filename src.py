...
if __name__ == '__main__':
    data = pd.DataFrame(np.random.normal(size=(100, 4)))
    evts = pd.Series(index=np.random.choice(np.arange(100), 5,
                                            replace=False),
                     data=1)
    es = EventStudy(data, evts, (-5, 5), 0)
    es.resample_wo_events(10)
